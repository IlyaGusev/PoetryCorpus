...
class MLAccentClassifier:
    ...
    def __init__(self, model_dir: str, accents_dict: AccentDict) -> None:
        """
        :param model_dir: папка с сохранёнными классификаторами.
        :param accents_dict: словарь ударений.
        """
        if not os.path.exists(model_dir):
            os.mkdir(model_dir)
        for l in range(2, 13):
            if not os.path.isfile(os.path.join(model_dir, self.clf_filename.format(l))):
                self.__build_accent_classifier(model_dir, accents_dict, l)
        self.classifiers = {l: joblib.load(os.path.join(model_dir, self.clf_filename.format(l))) for l in range(2, 13)}
